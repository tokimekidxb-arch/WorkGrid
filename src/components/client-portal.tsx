...
const operationalSheetUrl = "https://docs.google.com/spreadsheets/d/1A70KUthjq17UvN2x7jTx1EwPTfBQlK-G_ljMLiL0QfE/edit";
...
function RecordsView({ notify }: { notify: (m: string) => void }) { const records = [["TL-LEAVE-0001","Annual leave for autumn term break","Manager","Active"],["TL-DOC-0001","Student safeguarding policy 2026","Completed","Completed"],["TL-EXP-0001","Science laboratory teaching materials","Finance","Pending"],["TL-INC-0001","Classroom projector interruption","Administrator","Rejected"],["TL-IT-0001","Configure classroom projector","Administrator","Active"]]; const sync = async () => { notify("Synchronizing TEST LTD records…"); try { const response = await fetch("/api/google/service-account/sync", { method: "POST" }); const result = await response.json() as { synced?: number; error?: string }; notify(response.ok ? `${result.synced ?? 0} workflow records synchronized to Google Sheets.` : result.error ?? "Synchronization failed."); } catch { notify("Synchronization failed. Check the local server configuration."); } }; return <section className="portal-panel table-panel"><div className="table-tools"><div><Search size={16} /><input placeholder="Search company records" /></div><button onClick={sync}><FolderSync size={14} /> Sync now</button></div><div className="record-tabs"><button className="active">All test records</button><button>Leave</button><button>Documents</button><button>Expenses</button><button>Incidents</button><button>IT support</button></div><div className="data-grid"><div><b>Reference</b><b>Request title</b><b>Current approver role</b><b>Status</b><b>Sheet sync</b></div>{records.map((record) => <div key={record[0]}><span>{record[0]}</span><span>{record[1]}</span><span>{record[2]}</span><em className={record[3] === "Completed" ? "complete" : ""}>{record[3]}</em><span>Mapped</span></div>)}</div></section>; }
...
function DriveView({ notify }: { notify: (m: string) => void }) { return <div className="drive-layout"><section className="portal-panel drive-card"><div className="drive-connected"><span className="google-g">G</span><span><p>CLIENT-OWNED STORAGE</p><h2>TEST LTD Google Drive</h2><small>WorkGrid service account has Editor access to this client-owned folder.</small></span><em><i /> Connected</em></div><div className="drive-resources"><div><FolderSync size={19} /><span><strong>TEST LTD</strong><small>Linked client root folder</small></span><a href={driveFolderUrl} target="_blank" rel="noreferrer">Open</a></div><div><Grid2X2 size={19} /><span><strong>TEST LTD - WorkGrid Data</strong><small>Five workflow tabs are mapped in this client-owned Sheet.</small></span><a href={operationalSheetUrl} target="_blank" rel="noreferrer">Open</a></div></div><div className="sync-summary"><span><strong>Connection</strong><small>Service account verified</small></span><span><strong>Backend sync</strong><small>Sheets API active</small></span><button onClick={() => notify("TEST LTD Drive and Google Sheet access verified.")}><FolderSync size={14} /> Verify</button></div></section><aside className="portal-panel ownership-card"><ShieldCheck size={24} /><h2>TEST LTD owns its data</h2><p>Documents, spreadsheets, and generated reports stay in TEST LTD’s Google account. WorkGrid stores only access configuration, workflow state, and Google file references.</p><button onClick={() => notify("Service account connection settings opened.")}><Link2 size={14} /> Manage connection</button></aside></div>; }
