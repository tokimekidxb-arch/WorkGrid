...
const workflowRows = [
  ["Leave request", "General template", "3 stages", "Active", "No runs"],
  ["Document approval", "General template", "3 stages", "Active", "No runs"],
  ["Expense reimbursement", "General template", "4 stages", "Active", "No runs"],
  ["Incident report", "General template", "4 stages", "Active", "No runs"],
  ["IT support request", "General template", "3 stages", "Active", "No runs"],
];
const formRows = [
...
function ClientOverview({ onNavigate }: { onNavigate: (v: ClientView) => void }) {
  return <div className="portal-stack"><section className="client-welcome"><span><Sparkles size={21} /></span><div><p>YOUR COMPANY WORKSPACE</p><h2>TEST LTD is ready for real staff and workflow submissions</h2><small>Five General workflows are configured. The Drive folder is linked; Google OAuth is the next connection step.</small></div><button onClick={() => onNavigate("workflows")}>View workflows</button></section>
    <section className="admin-metrics"><ClientMetric icon={Workflow} label="Active workflows" value="5" detail="Basic plan limit reached" tone="blue" /><ClientMetric icon={ClipboardCheck} label="Workflow runs" value="0" detail="No submissions yet" tone="orange" /><ClientMetric icon={Users} label="Staff seats" value="5" detail="1 assigned · 4 available" tone="violet" /><ClientMetric icon={FileText} label="Generated reports" value="0" detail="Created when runs close" tone="green" /></section>
    <section className="portal-grid two"><article className="portal-panel"><ClientPanelTitle title="Work in progress" subtitle="Real workflow submissions only" action="All workflows" onAction={() => onNavigate("workflows")} /><div className="running-list"><div><span className="run-icon">0</span><span><strong>No workflow submissions yet</strong><small>New runs will appear after staff submit a linked form.</small></span><em>Empty</em></div></div></article><article className="portal-panel"><ClientPanelTitle title="Quick actions" subtitle="Complete workspace setup" /><div className="client-actions"><button onClick={() => onNavigate("forms")}><FormInput size={18} /><span><strong>Review forms</strong><small>Five workflow inputs</small></span></button><button onClick={() => onNavigate("staff")}><Users size={18} /><span><strong>Invite staff</strong><small>Four seats available</small></span></button><button onClick={() => onNavigate("drive")}><FolderSync size={18} /><span><strong>Finish Google connection</strong><small>OAuth and Sheets setup</small></span></button></div></article></section>
  </div>;
}
...
function RecordsView({ notify }: { notify: (m: string) => void }) { return <section className="portal-panel table-panel"><div className="table-tools"><div><Search size={16} /><input placeholder="Search company records" /></div><button onClick={() => notify("Connect Google OAuth and create the client Sheet before syncing.")}><FolderSync size={14} /> Sync now</button></div><div className="record-tabs"><button className="active">Leave</button><button>Documents</button><button>Expenses</button><button>Incidents</button><button>IT support</button></div><div className="data-grid"><div><b>Reference</b><b>Request title</b><b>Current approver role</b><b>Status</b><b>Updated</b></div><div><span>—</span><span>No synchronized records</span><span>—</span><em>OAuth required</em><span>Never</span></div></div></section>; }
function ReportsView() { return <section className="portal-panel table-panel"><div className="table-tools"><div><Search size={16} /><input placeholder="Search PDF reports" /></div><button>All workflows <ChevronDown size={14} /></button></div><div className="report-list"><div><span><FileText size={18} /></span><span><strong>No PDF reports yet</strong><small>A final report will be saved after a workflow run is completed.</small></span><em>0 files</em></div></div></section>; }
