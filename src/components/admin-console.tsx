...
const workflowTemplates: TemplateItem[] = [
  ...
  ["Purchase order", "Procurement", "Trading & Distribution", 4], ["Stock reorder", "Inventory", "Trading & Distribution", 3], ["Supplier onboarding", "Operations", "Trading & Distribution", 4], ["Delivery confirmation", "Logistics", "Trading & Distribution", 3],
  ["Stock transfer", "Inventory", "Retail", 3], ["Refund approval", "Finance", "Retail", 3], ["Store opening checklist", "Operations", "Retail", 5],
  ["Quality inspection", "Quality", "Manufacturing", 4], ["Maintenance request", "Maintenance", "Manufacturing", 4], ["Production change", "Operations", "Manufacturing", 5],
  ["Vehicle maintenance", "Fleet", "Logistics", 4], ["Delivery exception", "Logistics", "Logistics", 3], ["Tenant onboarding", "Operations", "Real Estate", 5], ["Property maintenance", "Maintenance", "Real Estate", 4],
  ["Guest complaint", "Service", "Hospitality", 4], ["Hospitality procurement", "Procurement", "Hospitality", 4], ["Patient intake review", "Operations", "Healthcare", 4], ["Course approval", "Academic", "Education", 4],
];
...
function TemplatePreview({ template, onClose, notify }: { template: TemplateItem; onClose: () => void; notify: (m: string) => void }) {
  ...
  return <div className="client-modal-backdrop" onMouseDown={onClose}><section className="client-modal template-preview workflow-designer" onMouseDown={(event) => event.stopPropagation()}><header><div><p>WORKFLOW TEMPLATE CONFIGURATION</p><h2>{template[0]}</h2><span>{template[2]} · {template[1]} · Draft template</span></div><button aria-label="Close" onClick={onClose}><X size={18} /></button></header><nav className="designer-tabs">{(["diagram", "approvers", "permissions", "outputs"] as const).map((item) => <button className={configTab === item ? "active" : ""} key={item} onClick={() => setConfigTab(item)}>{item}</button>)}</nav>
    {configTab === "diagram" && <div className="designer-body"><div className="flow-legend"><span><i className="source" /> Input</span><span><i className="action" /> Role action</span><span><i className="decision" /> Condition</span><span><i className="output" /> Output</span></div><section className="workflow-map" aria-label="Purchase approval routing diagram"><div className="flow-node source"><FileInput size={18} /><small>INPUT SOURCE</small><strong>Purchase request form</strong><em>Submitted by: Requester role</em></div><ArrowRight className="flow-arrow" size={21} /><div className="flow-node action"><UserCog size={18} /><small>APPROVER ROLE</small><strong>Department manager</strong><em>Can approve or reject</em></div><ArrowRight className="flow-arrow" size={21} /><div className="flow-node decision"><GitBranch size={18} /><small>ROUTING CONDITION</small><strong>Amount above AED 5,000?</strong><em>Yes → Finance · No → Complete</em></div><div className="flow-branch yes"><span>YES</span><ArrowDown size={18} /><div className="flow-node action"><ShieldCheck size={18} /><small>APPROVER ROLE</small><strong>Finance approver</strong><em>Can approve, reject, return</em></div></div><div className="flow-branch no"><span>NO</span><ArrowDown size={18} /><div className="flow-node output"><Check size={18} /><small>WORKFLOW STATUS</small><strong>Approved</strong><em>Lock final form values</em></div></div><div className="flow-output-row"><div className="flow-node output"><FolderOpen size={18} /><small>GOOGLE DRIVE</small><strong>Save attachments</strong><em>Client-selected folder</em></div><ArrowRight className="flow-arrow" size={21} /><div className="flow-node output"><FileText size={18} /><small>FINAL OUTPUT</small><strong>Generate close PDF</strong><em>Save PDF and approval history</em></div></div></section></div>}
